...
- Issue tạo theo nhóm: title `[CRM] <mô tả>`, labels `crm,feature,frontend,status::todo,priority::high` (+`shell,shared` cho context selection; `sale` cho partner), milestone patch tuần sau (v1.0.x — tạo milestone mới nếu chưa có), `due_date` độc lập (deadline có thể trước due của milestone).
...
- Snapshot issue map 08/2026: #182 context selection, #183 partner+staff, #184 authorization admin (luukhoahoc); #108 Orders, #109 CA Dossier (QuyCN) → milestone v1.0.5.
